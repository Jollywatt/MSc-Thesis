...
Since \cref{def:fibre-bundle} takes place in the category of manifolds, the projection $π : 𝒜 → ℳ$ is continuous; and in a \textdef{smooth fibre bundle}, the projection $π$ is differentiable and $F$, $ℱ$ and $ℳ$ are all smooth manifolds.
...
Hence, it is always possible to assign the local coordinates $\set{x^μ, x^a}$ to the bulk of a fibre bundle, where $x^μ$ are coordinates on the base and $x^a$ are coordinates on the fibres, and where $x^μ$ do not vary along the fibres.
...
In the language of fibre bundles, a field $f : ℳ → A$ becomes a \emph{section}, which is a map $f : ℳ → 𝒜$ into the bulk $𝒜$ such that $f(x) ∈ A_x$.
\begin{definition}
