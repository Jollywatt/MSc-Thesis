...
\begin{theorem}
	...
	,\end{align}
	where $ℛ$ is a volume containing $𝒙$ with smooth boundary $∂ℛ$ and volume $|ℛ| = \int_ℛ \bd V$.
...
\begin{proof}
	It will suffice to prove the case where $ℛ$ is a box shape.
	\todo{…cubes aren't smooth…}

	Let $B_ε = \set{x^i\ve_i | x^i ∈ [-ε, +ε]}$ denote the $n$-dimensional cube of diameter $2ε$ centred at the origin.
	If the surface $∂B_ε$ is oriented outward, then the face in the $+\ve^k$ direction has the orientation defined by the $(n-1)$-blade $\vol \ve^k$.
	Upon this face an infinitesimal surface element is
	\begin{align}
