...
It is easy to translate from the language of exterior calculus to tensor calculus, and vice versa, by identifying the former as the subalgebra of totally antisymmetric tensors (as in \cref{sec:exterior-algebra-as-antisymmetric}).
We will employ the Spivak convention, which in particular identifies $2$-forms via
\begin{align}
	\ve^μ ∧ \ve^ν ≡ \ve^μ ⊗ \ve^ν - \ve^ν ⊗ \ve^μ
\end{align}
where $\ve^μ$ are spacetime basis vectors with dimension $S$.
We then define the electromagnetic bivector as
\begin{math}
	\mathcal{F} = \frac12 F_{μν} \ve^μ ∧ \ve^ν
.\end{math}


Since the charge density $J \sim QS^{-3}$ has dimensions of charge per spacetime $3$-volume, it is natural to interpret it as a trivector
\begin{align}
	\mathcal{J} = J^{μνλ} \, \ve_μ ∧ \ve_ν ∧ \ve_λ ≔ J^μ \star \ve_μ = \frac1{3!}ε_{μνλα}J^α \ve^μ ∧ \ve^ν ∧ \ve^λ
\end{align}
so that the coefficients $J^{μνλ} \sim Q$ simply have dimensions of charge.\sidenote{
...
\begin{proof}
	...
	The BianFchi identity can be rewritten as
	\begin{align}
		∂_μG^{μν}
...
.\end{empheq}
\begin{proof}
	The multivector equation $\vd F = μ_0J$ separates into vector 
	\begin{math}
		{\vd \cdot F} = μ_0\vb J
	\end{math}
	and trivector
...
In the inertial frame with velocity $\vg_0$, the electromagnetic bivector is\sidenote{
	This can be easily shown from \cref{eqn:ga-em-bivector} and \cref{eqn:components-of-electromagnetic-tensor} using the identity 
	\begin{math}
		\frac12 ε^{ijk} \vg_i\vg_j = \vol \vg^0\vg^k = \vol \vg_k\vg_0
	.\end{math}
}
\begin{align}
	\label{eqn:F-is-E-iB}
...
and
\begin{math}
	\vol\vec B = \frac12 B_i ε^{ijk}\vs_j\vs_k
...
Similarly, the current density spacetime vector $J$ may be viewed under the space/time split by \paren{left}multiplication by the frame velocity $\vg_0$,
\begin{align}
	\vg_0 J = -ct + \vec J
.\end{align}
Right-multiplicatin yields $J \vg_0 = -ct - \vec J$
