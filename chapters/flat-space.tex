...
In a flat geometry, we make the assumtions that
\begin{itemize}
	\item points in spacetime are elements of a vector space, and differences of points are physically meaningful; and
	\item fields are parametric functions of a single vector which represents a point in spacetime.
\end{itemize}
For instance, the electromagnetic bivector in flat space $F : \RR^4 → \EA[2]{\RR^4}$ is a function between fixed vector spaces.
These assumptions are acceptable in special relativity, but in arbitrary regions of spacetime in the presence of gravity, the presence of curvature prevents spacetime from admitting a meaningful vector space structure.
...
This derivative will be defined when there is a canonical inclusion $ι : V^* → A$ of dual vectors into the algebra $A$, which is certainly true if $A$ is a quotient of $\TA{(V^*)}$.
