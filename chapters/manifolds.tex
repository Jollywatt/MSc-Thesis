...
This calls for manifold geometry.
...
\begin{proof}
	...
	.\end{align}
	Using \eqref{eqn:lift-of-base-tangent-vectors}, the projection operator at $p ∈ ℱ$ can be written as
	\begin{align}
...
The covariant derivative in \eqref{lem:covariant-derivative-rewritten} acting on a section $f : ℳ → ℱ$ in the direction $𝒖 ∈ \TT_x ℳ$ has the full form
\begin{align}
	∇_𝒖f ≔ ∇f(𝒖) = \dd f(𝒖) + Γ_f(𝒖)
.\end{align}
The connection map $Γ_f : \TTℳ → H ⊆ \TT ℱ$ is linear in $𝒖$, so we may write it as a bitensor (matrix-valued) section $Γ_f : ℳ → \TT^*ℳ ⊗ \TT ℱ$, which is equivalently a $\TT ℱ$-valued $1$-form.
...
Thus, in the coordinate basis, the covariant derivative acting on a vector $𝒖 ∈ \TT_xℳ$ is
\begin{align}
...
However, since $∇f(𝒖) ∈ \VVℱ$ is vertical, we expect its horizontal components $∇_μ f^ν$ to vanish, leaving only vertical components $∇_μf^A \∂_A = ∇_μf^a \∂_a$.
...
,\end{align}
and so \eqref{eqn:covariant-derivative-coords.2} implies $∂_μ f^ν = -Γ^ν{}_μ$.
Hence, the base components of the two terms in \eqref{eqn:covariant-derivative-coords.3} cancel, leaving only the fibre components,
\begin{align}
...
In practice, it is usual to have a (local) trivialisation and have $f : ℳ → F$ be given as a field, in which case $\dd f = \dd f^a \∂_a ∈ \VVℱ$ instead.
...
This is usually how computations are performed --- but to reiterate, the expression \eqref{eqn:covder-of-trivialised-field} with $f : ℳ → F$ depends on a (local) trivialisation, whereas the previous sense of covariant derivative in which $f : ℳ → ℱ$ is purely geometrical.