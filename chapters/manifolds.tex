...
\begin{definition}
	...
	This means for every point $x ∈ ℳ$ there exists a neighbourhood $x ∈ \manif U \subseteq ℳ$ and subset $U \subseteq \RR^n$ with a homeomorphism\sidenote{continuous bijection with continuous inverse} $φ : \manif U \biject U$, called a \textdef{coordinate chart}, between them.
	...
	\end{math}
	between coordinate charts $φ : 𝒰 \biject U$ and $ϕ : 𝒱 \biject V$ are smooth (meaning infinitely differentiable).
\end{definition}
...
\begin{definition}
	...
	The \textdef{tangent space \,$\TT_xℳ$} of a smooth manifold at a point $x ∈ ℳ$ is the vector space of scalar derivatives at that point.\sidenote{
...
\begin{definition}
	...
	The \textdef{differential} or \textdef{push forward} $\dd f ≡f_*$ of a map $f : ℳ \to \manif N$ between smooth manifolds is the map $\dd f : \TTℳ \to \TT\manif N$ defined by
	\begin{align}
...
In the definition above, vectors act on scalar functions as derivations; hence $\dd f(𝒖)$, a vector, is defined by its action on an arbitrary scalar field.
...
If $𝒖|_x ∈ \TT_xℳ$ is now a family of vectors defined everywhere over $ℳ$, then $\dd f(𝒖)|_{f(x)} = \dd f(𝒖|_x)$ is defined only at each $f(x) ∈ \manif N$.
...
% (In other words, as will be defined, they depend on the choice of \emph{trivialisation} of the tangent bundle.)
...
For a concrete example of why this is necessary, take fluid flow on the sphere $\Sphere^2$.
...
Any representation of the fluid flow as a field $f : \Sphere^2 → \RR^2$ is only defined after the fixed codomain $\RR^2$ is identified with each geometrically--distinct tangent plane on the sphere.
Notice, such an identification is not canonical.
Even worse, it is not even possible to do this smoothly everywhere on the sphere\sidenote{
	\emph{Proof.} Consider a constant non-zero vector field $f(x) = 𝒖 ∈ \RR^2$.
	If all tangent spaces are smoothly identified with $\RR^2$, then $f$ represents a fluid flow on $\Sphere^2$ which is smooth and nowhere vanishing.
...
The vector field $f$ on the sphere now becomes a \emph{section} of $\TT \Sphere^2$, or a map $g : \Sphere^2 → \TT \Sphere^2$ such that $g(x)$ belongs to the tangent space at $x$.
No longer is the expression $g(x) + g(y)$ well-defined.
...
\begin{marginfigure}
	...
	\caption{
		...
		(b) A fibre bundle $\fibrebundle F ℱ ℳ$ with a section $f ∈ \secs(ℱ)$ whose individual fibres $F_x$ are labelled by base point $x$.
	}
	\label{fig:fibre-bundle}
\end{marginfigure}

\Cref{def:fibre-bundle} takes place in the category of manifolds, so the projection $π : ℱ → ℳ$ is assumed continuous.
...
This requires locally splitting the bulk $ℱ → ℳ × F$ into its base and fibre components, identifying each fibre with $F$ so its $\set{x^a}$ coordinates carry over to all fibres.
This splitting, if it can be done globally, is known as a (global) \emph{trivialisation} of the bundle.
\begin{definition}
...
In other words, local trivialisations are equivalent to local coordinates $\set{x^μ, x^a}$.
...
In the language of fibre bundles, a field $f : ℳ → F$ is replaced by a \emph{section}, which is a `vertical' map\sidenote{The adjectives `vertical' and `horizontal' are used in reference to e.g., \cref{fig:fibre-bundle}, where fibres are drawn as vertical stalks over a horizontal base manifold.α	} $f : ℳ → ℱ$ into the bulk $ℱ$ such that $f(x) ∈ F_x$.
\begin{definition}
...
A general procedure to convert locally defined objects into structures on a manifold is to form the associated bundle, with operations acting pointwise on sections.
...
\end{math}
of $\secs(ℰ)$-valued exterior forms, for some vector bundle $\fibrebundle V {ℰ} ℳ$.
...
Generally, tangent vectors and objects built on top of the tangent bundle are flowable.
...
\begin{definition}
	\label{def:flow}
	The \textdef{flow} of $𝒖 ∈ \secs(\TT ℳ)$ is the $1$-parameter family of diffeomorphisms $\flow{𝒖}{t} : ℳ → ℳ$ satisfying
	\begin{align}
		\dv{t}\eval{\flow{𝒖}{t}(x)}_{t=0} = 𝒖|_x
	\end{align}
...
\begin{math}
	\flow{𝒖}{t} f ≔ e^{-t𝒖} f
.\end{math}
For example, if $\manif M = \RR$ is one dimensional, $\flow{∂_x}{t}f = e^{-t∂_x}f(x) = f(x - t)$ is the Taylor series of $f$ translated by $+t$.
Tangent vectors $𝒗 ∈ \TT ℳ$ are also flowable, using the differential of a flow
\begin{math}
	\dd(\flow{𝒖}{t}) : \TT ℳ → \TT ℳ
.\end{math}
Specifically, we define the flow of tangent vectors
\begin{align}
...
\begin{lemma}
	The Lie derivative on scalar functions is
	\begin{math}
		\lie_𝒖 f = 𝒖(f)
	,\end{math}
	and on tangent sections is the Lie bracket,
	\begin{math}
		\lie_𝒖 𝒗 = [𝒖, 𝒗] ≔ 𝒖 ∘ 𝒗 - 𝒗 ∘ 𝒖
	.\end{math}
\end{lemma}
\begin{proof}
	...
	For tangent sections, unpacking \cref{def:lieder} for a vector argument, and then using \cref{def:differential} to rewrite the pushforward, we have
	\begin{align}
		(\lie_𝒖 𝒗)f|_x
