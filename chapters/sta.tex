...
Both metric signatures $η = \op{diag}\sig{-+++}$ and $η = \op{diag}\sig{+---}$ are appropriate for relativistic physics, and both are used in literature.
Either choice is acceptable, though expressions written in the STA are generally not independent of this overall sign.
...
If this equality is to be preserved (and not deformed by an overall sign) then the relative vectors must be defined as $\vs_i = \vg_0\vg_i$ or $\vs_i = \vg_i\vg_0$ according to the choice of metric signature.

Another notable difference is in the space/time split of a position vector $X ∈ \GA[1](1, 3)$ into components $X^0 = ct$ and $(X^i) = \vec x$, achieved by multiplication with the frame velocity $\vg_0$.
For example, the equations
\begin{align}
	X \vg_0 &= ct + \vec x
,&	\vg_0 X &= ct - \vec x
\end{align}
valid in the $\sig{+---}$ signature both change by an overall factor of $-1$ in the $\sig{-+++}$ signature (see )
