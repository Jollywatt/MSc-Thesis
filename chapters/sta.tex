...
While the overall physics is invariant, expressions written in the STA are generally not independent of this choice of sign.
...
,&	\vg_0 𝑿 &= ct - \vec x
\end{align}
are valid in the $\sig{+---}$ signature, but both change by an overall sign in the $\sig{-+++}$ signature.
...
The choice of metric signature sign may be rendered irrelevant by using sign-agnostic definitions and expressions.
...
\end{align}
The spacetime vector derivative $\vd = \vg^μ ∂_μ$ decomposes into a scalar time derivative and the spatial derivative $\vec ∇ = \vs^i ∂_i$.
