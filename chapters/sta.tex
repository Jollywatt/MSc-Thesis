...
Multivectors of any kind which square to zero are called \textdef{null} or \textdef{lightlike}.
...
We single out $\sqrt{-1} = \vol$ as `the' principal root as this proves to be useful.\sidenote{
	In electromagnetism, the imaginary unit $i$ often plays the role of $\vol$, e.g., with Riemann--Silberstein vector \cite{silberstein}, where $i$ and $\vol$ are Hodge-like duals \cite{dressel2015sta}.
}
...
Such a \textdef{space\slash time split} identifies \emph{even} multivectors in the spacetime algebra $\GA[+](1,3)$ with $\GA(3)$ multivectors, and provides an efficient, purely algebraic method for switching between inertial frames \cite{hestenes2003sta}.
...
For example a spacetime bivector $F = F^{μν}\vg_μ\vg_ν$ may be separated into timelike $F^{i0}$ and spacelike $F^{ij}$ components and viewed as a $\qty{1,2}$\hyp multivector in $\GA(3)$.
...
\Cref{eqn:equivalent-rotors} also shows that every Lorentz rotor $±e^{σ_0}$ is equal to a pure bivector exponential $e^{σ_n}$ with a shifted rotational part $λ_- \mapsto λ_- + n\pi$.
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item \textdef{loxodromic} if $σ^2 = α + \vol β$ is not a scalar but a $\set{0, 4}$\hyp multivector.
	\end{itemize}
	% If $σ^2 = α + \vol β$ is not a scalar but a $\set{0, 4}$\hyp multivector, then call $σ$ \textdef{loxodromic}.
\end{definition}
