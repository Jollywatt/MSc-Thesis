...
\begin{definition}
	...
	If $A = \bigoplus_{k ∈ R} A_{k}$ is an $R$-graded algebra, then an element $a ∈ A$ is \textdef{homogeneous} if it belongs to some $A_k$, in which case it is said to be a \textdef{$k$-vector}.\sidenote{
		We use the aliases \emph{bivector} for $2$-vector, \emph{trivector} for $3$-vector, \emph{scalar} for $0$-vector, etc.
	}
	If $a ∈ A_{k_1} ⊕ \cdots ⊕ A_{k_n}$ is inhomogeneous, we may call it a $\set{k_1, ..., k_n}$-multivector.
\end{definition}

All elements of a graded algebra are either inhomogeneous or a $k$-vector for some $k$, and every non-zero $k$-vector is a sum of one or more irreducible terms, called \emph{blades}, in the following sense.
...
One of the simplest algebras to construct as a quotient of the tensor algebra, yet still one of the most useful, is the \emph{exterior algebra}, first popularised by Hermann Grassmann in 1844.
\begin{definition}
	...
	.\end{align}
	The product in $\EA{V}$ is denoted `$∧$' and is called the \textdef{wedge product}.
\end{definition}
...
The ideal $\gen{𝒖 ⊗ 𝒖}$ is homogeneous with respect to the $\ZZ$-grading of the parent tensor algebra,\sidenote{
	This follows because $\gen{𝒖 ⊗ 𝒖}$ is generated by grade $2$ elements $𝒖 ⊗ 𝒖 ∈ \TA[2]{V}$.
} and hence $\EA{V}$ is itself $\ZZ$-graded by \cref{thm:quotient-algebra-by-homogeneous-congruence}.
...
\end{definition}
Notably, the definition is independent on the particular decomposition of the blade as a wedge product of vector.
...
.\end{align}
However, it \emph{is} a homomorphism if we instead equip $S ≡ (S, ∧)$ with the antisymmetrising product $∧ : S × S → S$ defined by
\begin{align}
...
We \emph{could} use the exterior algebra $\EA{V^*}$ over the dual space of linear maps $V → \RR$ as a model for exterior forms, as follows.
...
\begin{definition}
	...
	For another vector space $A$, an \textdef{$A$-valued $k$-form} $φ ∈ \forms[k](V, A)$ is such a map with codomain $A$ instead of $F$.
\end{definition}
...
\begin{definition}
	...
	The \textdef{volume element} $\vol ∈ \EA[n]{V}$ of the metrical exterior algebra $\EA{V}$ is the unique (up to sign) pseudoscalar satisfying $\ip{\vol, \vol} = 1$.
\end{definition}
...
The Hodge dual $\star : \EA[k]{V} → \EA[n - k]{V}$ associates each pair of fixed-grade subspaces of the same dimension; in particular, scalars with pseudoscalars via $\star 1 = \vol$.
