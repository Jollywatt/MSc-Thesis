...
Indeed, some authors omit the normalisation factor, which has the effect of changing \cref{eqn:wedge-as-antisymm} to\sidenote{
	Written with $\op{Sym}^-$ \emph{including} the factor $\frac1{k!}$, as in \eqref{eqn:(anti)symmetriser}.
}
\begin{align}
...
\begin{enumerate}
	...
	This forbids the proper extension of $\EA{V^*}$ to non-Abelian vector-valued forms, where the values $\ve^i(𝒖_j)$ may not commute.
...
\end{enumerate}
Thus, we will not define exterior forms in terms of the exterior algebra $\EA{V^*}$, to better agree with standard convention.
...
Throughout, we will not have need to consider degenerate metrics,\sidenote{
	Degenerate signatures find use in computer graphics, especially \emph{projective geometric algebra} \cite{hestenes1991pga,vince2008ga-graphics}.
} and so assume non-degeneracy.
...
A useful duality operation can be defined in an exterior algebra $\EA{V}$ with a metric, which relates the $k$- and $(n - k)$-grade subspaces.
\begin{definition}
...
The Hodge dual $\star : \EA[k]{V} → \EA[n - k]{V}$ associates each pair of fixed-grade subspaces of the same dimension; in particular, scalars with pseudoscalars, $\star 1 = \vol$.

\todo{Seems incomplete...}