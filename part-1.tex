...
By associativity, it follows that $𝒗_1 ∧ 𝒗_2 ∧ \cdots ∧ 𝒗_k$ vanishes exactly when the $𝒗_i$ are linearly dependent.\sidenote{
	\begin{proof}
		Blades of the form $a = \dotted{𝒖_\i} ∧ k$ vanish when two or more vectors are repeated.
...
\begin{align}
	...
	\op{Sym}^±(\dotted{𝒖_\i} ⊗ k) = \frac1{k!} \sum_{σ ∈ S_k} (±1)^σ \dotted{𝒖_{σ(\i)}} ⊗ k
\end{align}
...
Furthermore, note that the kernel of $\op{Sym}^-$ consists of tensor products of linearly dependent vectors, and sums thereof,\sidenote{
	\begin{proof}
		If $a = \dotted{𝒖_\i} ⊗ k$ where two vectors $𝒖_i = 𝒖_j$ are equal, then $\op{Sym}^-(a) = 0$ since each term in the sum in \cref{eqn:(anti)symmetriser} is paired with an equal and opposite term with $i ↔︎ j$ swapped.
...
\begin{align}
	\ker \op{Sym}^- = \spanof{\dotted{𝒖_\i} ⊗ k | k ∈ \NN, \text{$\set{𝒖_i}$ linearly dependent}}
,\end{align}
...
\subsubsection{Exterior forms}

The exterior algebra is most frequently encountered in physics in the theory of \emph{exterior (differential) forms}, which are alternating multilinear maps from a vector space.

We may wish to use the exterior algebra $\EA{V^*}$ over the dual space of linear maps $V → \RR$ as a model for exterior forms.
Using the dual basis $\set{\ve^i} ⊂ V^*$, any element $f ∈ \EA[k]{V^*}$ has the form $f = f_{\dotted{i_\i}{}k} \dotted{\ve^{i_\i}} ∧ k$, and each component acts on $\TA[k]{V}$ as
\begin{align}
	\label{eqn:poor-mans-exterior-form}
	(\dotted{\ve^{i_\i}} ∧ k)(\dotted{𝒖_\i} ⊗ k) = \frac1{k!} \sum_{σ ∈ S_k} (-1)^σ\dotted{\ve^{i_{σ(\i)}}(𝒖_\i)}{}k
.\end{align}
...
\end{enumerate}
Thus, we define exterior forms separately to the exterior algebra of covectors.
...
\end{definition}
The evaluation of a form is denoted $φ(\dotted{𝒖_\i} ⊗ k)$ or $φ(\dotted{𝒖_\i} , k)$, and the wedge product of a $p$-form $φ$ and $q$-form $ϕ$ is defined (in the Spivak convention)
\begin{align}
...
\begin{align}
 	(\dotted{\dx^{μ_\i}} ∧ k)(\dotted{𝒖_\i} ⊗ k) = \sum_{σ ∈ S_k} (-1)^σ \dotted{\dx^{μ_\i}(𝒖_{σ(\i)})}{}k
.\end{align}
% \begin{align}
% 	φ ∧ ϕ = \op{Alt}(φ ⊗ ϕ)
% \end{align}
...
% \begin{align}
% 	\op{Alt}(f)(\dotted{𝒖_\i} ⊗ k) = k! f(\op{Sym}^-(\dotted{𝒖_\i} ⊗ k))
% \end{align}
